...
function notifyOfSuccess(){
  MailApp.sendEmail("mufagamescalendar@gmail.com", "MUFA calendar ran successfully", "Yay good job");
}

function notifyOfResultsWithErrors(resultsDictionary, teamsByDivision){
  let message = "Well something went wrong. Here's what:\n\n";

  message = message.concat(`Target teams: ${JSON.stringify(teamsByDivision)}\n\n`)

  for (const [divisionId, teamResultList] of Object.entries(resultsDictionary)){
  ...
  console.log(message);
  MailApp.sendEmail("mufagamescalendar@gmail.com", "MUFA calendar ran with errors", message);
}