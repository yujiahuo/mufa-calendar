...
function getEventsFromPageViaTable(htmlContent, myTeamName, errors) {
  const events = [];

  // Get the table of games
  const TABLE_REGEX = /<table class="rgMasterTable"[\s\S]*?<tbody>([\s\S]*?)<\/tbody>/;
  ...
  for (const rowMatch of rowMatchList) {
    if (rowMatch.length > 1){
      ...
      if (!opponentWithWinHistory) errors.add("Unable to extract opponent info");
      ...
      events.push({
        ...
        "description": getDescription(ourJersey, theirJersey, field, diagramAnchor)
      });
    }
  }
